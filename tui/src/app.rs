pub struct App<T: HandleTransport> {
    // --- state ---
    finished: bool,
...
impl<T: HandleTransport> App<T> {
    pub fn new(session: session::ReceiveHello<T>) -> Self {
        // TOOD: start session in new thread and simply report back updates
