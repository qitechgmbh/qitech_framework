...
use qitech_framework::machine::resource::{ConfigProperty, Measurement, StateProperty};
...
pub struct TraverseController {
    ...
    // --- state ----
    is_going_in: StateProperty<bool>,
    is_going_out: StateProperty<bool>,
    is_homed: StateProperty<bool>,
    is_going_home: StateProperty<bool>,
    is_traversing: StateProperty<bool>,
...
impl TraverseController {
    ...
    fn distance_to_position(&self, target_position: Length) -> Length {
        if self.position > target_position {
            ...
            target_position - self.position.get()
        } else {
            Length::ZERO
        }
    }
    ...
    pub fn sync_position(&mut self, traverse: &dyn StepperVelocityEL70x1Device) {
        ...
        self.position.set(self.microstep_converter.steps_to_distance(steps as f64));
    }
