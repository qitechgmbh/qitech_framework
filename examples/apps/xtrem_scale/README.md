...
## Finding your modules' device ids

Each `.xtrem_device(..)` line claims one module by its **device id** (register `0001h`) — the
address the bus routes replies on. Modules ship as `01`, so every module needs a distinct id
...
Do this one module at a time. While several share an id, a broadcast write renumbers all of them
at once.

To read the ids currently on the bus, start this app and look at the `XtremDiscoveryCompleted`
init event — it lists every module that answered the sweep, claimed or not, with its id, serial,
and address. Or probe directly:

```bash
cargo run -p xtrem --example discover -- --bind 0.0.0.0:5555 --broadcast <subnet>.255:4444
```

Put the id into `SCALE_DEVICE_ID` in [src/main.rs](src/main.rs).

## Adding a scale

`.machine::<ScaleV1>()` registers the machine *type* once; each `.xtrem_device` line claims one
module for one `MachineIdentificationUnique`, so N lines produce N machines. To add a second:

1. Give the new module a unique device id with `assign_ids`, as above.
2. Add `.xtrem_device::<XtremScale>(0x04, ScaleV1::IDENTIFICATION.unique(2), ScaleMode::Poll)`.

Step 1 is not optional. Because the bus routes replies by `ID_O`, a shared id does not identify a
module at all — both would feed their readings into both drivers. Init detects this during the
sweep, reports `XtremDeviceIdCollision`, and skips the affected modules rather than publishing
wrong weights.
